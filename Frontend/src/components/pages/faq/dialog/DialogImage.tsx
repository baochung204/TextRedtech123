...
const DialogImage: React.FC<PropsDialog> = ({
  ...
  const [dataImage, setDataImage] = useState<{ id?: string, dateTime?: string, imageURL: string; name: string; title: string; description?: string }[]>([]);
                ...
                {dataImage.map((item, index) => (
                  <>
                    <Grid item xs={12} lg={4} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }} >
                      <Avatar sx={{ width: '190px', height: '190px' }} src={item.imageURL} alt={item.imageURL} />
                    </Grid>
                                  ...
                                  <Grid item xs={12} key={index}>
                                    <Grid container spacing={2}>
                                      <Grid item xs={1.5} sx={{ display: 'flex', alignItems: 'center' }}>
                                        <Avatar src={item.imageURL} alt={item.imageURL} variant="rounded" sx={{ width: 56, height: 56 }} />
                                      </Grid>
