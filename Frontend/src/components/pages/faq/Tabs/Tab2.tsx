...
//             <CardContent onClick={() => handleClick(items)} sx={{ cursor: 'pointer' }}>
...
import { fetchFunctions } from 'src/store/user/user-resources/functions/functionsUseSlice';
import {
  DataType,
  FunctionsType,
} from 'src/store/user/user-resources/functions/type/functionsType';
...
const Tab2 = () => {
  ...
  const functions = useSelector((state: AppState) => state.resourcesFunctions.data);
  const [functionsData, setFunctionsData] = useState<FunctionsType>();
  useEffect(() => {
    dispatch(fetchFunctions({ page, size: rowsPerPage }));
  }, [dispatch, page, rowsPerPage]);
  useEffect(() => {
    if (functions !== functionsData) {
  ...
  const totalElements = functionsData?.totalElements || 0;

  return (
    <>
      <Grid container spacing={2}>
        {functionsData?.content?.map((item: DataType) => (
          <Grid item xs={12} sm={6} md={4} key={item.functionId}>
            <CardContent onClick={() => handleClick(item)} sx={{ cursor: 'pointer' }}>
              <Stack direction="row" gap={2} alignItems="center">
                <Avatar alt={item.functionName} src={item.badgeUrl} />
                <Box>
                  <Typography variant="h6" noWrap>
                    {item.functionName}
                  </Typography>
